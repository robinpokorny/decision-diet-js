import { Head, Notes, Appear, Split, Image, Horizontal } from "mdx-deck";
import { prism } from "@mdx-deck/themes";
import BigBlock from "./lib/BigBlock";
import FullImage from "./lib/FullImage";
import theme from "./lib/theme";

export const themes = [prism, theme];

<Head>
  <title>
    Decision-Making Diet in a JavaScript Developer’s Life – Robin Pokorny
  </title>
  <meta name="twitter:card" content="summary_large_image" />
  <meta name="twitter:creator" content="@robinpokorny" />
  <meta name="og:image" content="https://decision-diet-js.now.sh/deck.png" />
  <meta
    property="og:title"
    content="Decision-Making Diet in a JavaScript Developer’s Life – Robin Pokorny"
  />
</Head>

## Practising the

# **Decision-Making Diet**

## in a JavaScript Developer’s Life

_by [Robin Pokorny](https://robinpokorny.com) _

---

## How many decisions do you make in a day?

---

<Split>
  
<Appear>

Should I hit snooze?

What should I wear today?

Is the shirt still clean?

What will I eat for breakfast?

Is two-days open salmon OK?

Tram or bike?

Should I let her sit?

Do I reply to this email now?

</Appear>

&nbsp;

</Split>

---

<Split>
<>

Should I hit snooze?

What should I wear today?

Is the shirt still clean?

What will I eat for breakfast?

Is two-days open salmon OK?

Tram or bike?

Should I let her sit?

Do I reply to this email now?

</>

What will I work on first?

<Appear>

Is my standing desk the right height?

How to name this variable?

Is decorator pattern OK here?

How to iterate this object?

Should I split this function?

Should I comment this line?

</Appear>

</Split>

---

<BigBlock>

# 35 000

<Appear>

a ton

</Appear>

</BigBlock>

---

import battery from "./img/battery.png";

<Split>

<Image src={battery} height="70vh" size="contain" />

<Appear>

<div>

### decision making

### analytical thinking

### self-control

### imagination

</div>

</Appear>

</Split>

---

<!-- In HTML because of formatting -->

<h1>
  ‘Separating what <strong>really</strong> matters
  <br />
  from what <strong>sort of</strong> matters
  <br />
  from what <strong>doesn’t</strong> matter at all.’
</h1>

_~ Jason Fried and DHH_

<Notes>
  David Heinemeier Hansson
  <br />
  More creative, productive, and generally better in both life and code
</Notes>

---

# 3 strategies

# ✖️

### _`#life #development #javascript`_

<Notes>I will be giving you tasks</Notes>

---

<BigBlock>

# Precommit

</BigBlock>

<Notes>Impossible to fail, lock yourself into a virtuous path</Notes>

---

import barak_mark from "./img/barak_mark.jpg";

<FullImage src={barak_mark} />

<Notes>
  No matter how you feel about them.
  <br />
  They removed what sort of matters: clothes
</Notes>

---

import obama_naked from "./img/obama_naked.jpg";

<FullImage src={obama_naked} />

---

_`#life`_

## I eat the same breakfast

<Notes>Not clothes, decisionabout clothes</Notes>

---

import eggs from "./img/eggs.jpg";

<FullImage src={eggs} />

---

_`#life`_

## I eat the same breakfast

<br/>
<br/>
<br/>
<br/>

_/\*\* @todo Buy eggs and spinach \*/_

---

_`#development`_

## I adhere to a code style

<Appear>

| Language | Style           |
| -------- | --------------- |
| JS       | AirBnB/Standard |
| Python   | PEP-8           |
| PHP      | PSR-2           |
| CSS      | cssguidelin.es  |

_/\*\* @todo Try style you hate \*/_

</Appear>

---

_`#javascript`_

## I use Prettier

<br />

<Appear>

# Linter : Errors — Formatter : Style

</Appear>

---

import pubsub1 from "./img/pubsub1.png";
import pubsub2 from "./img/pubsub2.png";

<Split>

<Image src={pubsub1} width="40vw" height="90vh" size="contain" />

<Image src={pubsub2} width="40vw" height="90vh" size="contain" />

</Split>

---

_`#javascript`_

## I use Prettier

<br />
<br />
<br />

_/\*\* @todo Just install Prettier \*/_

---

<BigBlock>

# Plan

</BigBlock>

<Notes>What usually works and the things to try first</Notes>

---

# <span style={{ color: "#FF4136", textTransform: "uppercase" }}>red</span>

# <span style={{ color: "#0074D9", textTransform: "uppercase" }}>green</span>

# <span style={{ color: "#2ECC40", textTransform: "uppercase" }}>blue</span>

# <span style={{ color: "#B10DC9", textTransform: "uppercase" }}>blue</span>

---

# <span style={{ color: "#2ECC40", textTransform: "uppercase" }}>кра́сный</span>

# <span style={{ color: "#0074D9", textTransform: "uppercase" }}>зелёный</span>

# <span style={{ color: "#FF4136", textTransform: "uppercase" }}>кра́сный</span>

# <span style={{ color: "#B10DC9", textTransform: "uppercase" }}>си́ний</span>

---

# <span style={{ color: "#FF4136", textTransform: "uppercase" }}>blue</span>

# <span style={{ color: "#B10DC9", textTransform: "uppercase" }}>red</span>

# <span style={{ color: "#0074D9", textTransform: "uppercase" }}>green</span>

# <span style={{ color: "#2ECC40", textTransform: "uppercase" }}>red</span>

---

_`#life`_

## I choose a daily highlight

<Notes>
  When you look back on your day, what activity or accomplishment or moment do
  you want to savor?
</Notes>

---

import highlight from "./img/highlight.jpg";

<FullImage src={highlight} size="contain" />

<!-- @TODO use board -->

---

_`#life`_

## I choose a daily highlight

<br/>
<br/>
<br/>

_/\*\* @todo Set Monday highlight on Sunday \*/_

---

_`#development`_

## I stick to a paradigm

---

# Paradigm

## **==** what structures and when

<br />

<Appear>

## **Functional** / **Structural** / **Object-oriented**

</Appear>

---

_`#development`_

## I stick to a paradigm

<br />
<br />
<br />

_/\*\* @todo Agree on a paradigm in the team \*/_

---

_`#javascript`_

## I prefer arrow functions

## I iterate with map/filter/reduce

---

```js
// Function statement
function add(a, b) {
  return a + b;
}
// Function expression
const add = function(a, b) {
  return a + b;
};
// Arrow function
const add = (a, b) => a + b;
```

---

```js
const words = ["hello", " ", "world"];

for (let i = 0; i <= words.length; i++) {
  const word = words[i];
  if (word.length <= 0) continue;
  words[i] = uppercaseFirstLetter(word.trim());
}

const titlecasedWords = words
  .filter(isNotEmpty)
  .map(trim)
  .map(uppercaseFirstLetter);
```

<Notes>
  Mistake: should be `i &lt; length`
  <br />
  for .. of, for .. in
  <br />
  Switch order
</Notes>

---

_`#javascript`_

## I prefer arrow functions

## I iterate with map/filter/reduce

<br/>
<br/>
<br/>

_/\*\* @todo Avoid `function` and `for` \*/_

<Notes>Never used `finally` either</Notes>

---

<BigBlock>

# Divide

</BigBlock>

---

import freewrite from "./img/freewrite.jpg";

<a href="https://kit.co/robinpokorny/decision-making-diet/freewrite-distractio">
  <FullImage src={freewrite} />
</a>

<Notes>Write drunk, edit sober – EH</Notes>

---

import freewrite2 from "./img/freewrite2.jpg";

<a href="https://kit.co/robinpokorny/decision-making-diet/freewrite-distractio">
  <FullImage src={freewrite2} size="contain" />
</a>

<Notes>Write drunk, edit sober – EH</Notes>

---

_`#life`_

## I rediscovered Pomodoro

<Notes>analog pen, whiteboard</Notes>

---

import pomodoro from "./img/pomodoro.png";

<a href="https://kit.co/robinpokorny/decision-making-diet/the-pomodoro-techniq">
  <FullImage src={pomodoro} size="contain" />
</a>

---

_`#life`_

## I rediscovered Pomodoro

<br/>
<br/>
<br/>

_/\*\* @todo Try completing one Pomodoro \*/_

---

_`#development`_

## I follow an architecture

---

# Architecture

<Appear>

## **!==** microservices

## **==** drawing lines

</Appear>

<Notes>
  Architecture defines where the application performs its core functionality and
  how that functionality interacts with things like the database and the user
  interface.
</Notes>

---

# ‘A good architecture maximizes the number of decisions **not** made.’

_~ Robert C. Martin_

<Notes>Clean Architecture</Notes>

---

_`#development`_

## I follow an architecture

<br/>
<br/>
<br/>

_/\*\* @todo Draw your achitecture \*/_

---

import spotify from "./img/spotify.png";

_`#javascript`_

## I want to point-free

<Image src={spotify} width="32vw" height="8vw" size="contain" />

---

_`#javascript`_

## ~~I want to point-free~~

## I code point-free

<Notes></Notes>

---

```js
const add = (a, b) => a + b;

numbers.map(n => add(1, n));
```

<Appear>

<div>

```js
const add = a => b => a + b;

numbers.map(n => add(1)(n));
numbers.map(add(1));
```

</div>

<div>

```js
numbers.map(_.add(1)); // Lodash/fp
```

</div>

</Appear>

<Notes>John-David Dalton</Notes>

---

```js
const isOfAge = user => user.age > 17;
```

<br />

<Appear>

<div>

```js
const isOfAge = ({ age }) => age > 17;
```

</div>

</Appear>

---

_`#javascript`_

## I code point-free

<br/>
<br/>
<br/>

_/\*\* @todo Learn currying and destructuring \*/_

---

import make_time from "./img/make_time.jpg";
import willpower from "./img/willpower.jpg";
import crazy_at_work from "./img/crazy_at_work.png";

<Horizontal>

<a href="https://kit.co/robinpokorny/decision-making-diet/make-time-how-to-fo">
  <Image src={make_time} width="25vw" height="90vh" size="contain" />
</a>

<a href="https://kit.co/robinpokorny/decision-making-diet/willpower-rediscove">
  <Image src={willpower} width="25vw" height="90vh" size="contain" />
</a>

<a href="https://kit.co/robinpokorny/decision-making-diet/it-doesn-t-have-to-b">
  <Image src={crazy_at_work} width="25vw" height="90vh" size="contain" />
</a>

</Horizontal>

---

import matilda from "./img/matilda.jpg";

<Horizontal>

<Image src={matilda} width="30vw" height="90vh" />

</Horizontal>

<Notes>art director Matilda Kahl in NY, 3 years, 15x</Notes>

---

import matilda_wardrobe from "./img/matilda_wardrobe.jpg";
import matilda_weekend from "./img/matilda_weekend.jpg";

<Horizontal>

<Image src={matilda} width="30vw" height="90vh" />

<Image src={matilda_wardrobe} width="30vw" height="90vh" />

<Appear>

<Image src={matilda_weekend} width="30vw" height="90vh" />

</Appear>

</Horizontal>

<Notes>art director Matilda Kahl in NY, 3 years, 15x</Notes>

---

<!-- In HTML because of formatting -->

<h1>
  ‘Separating what <strong>really</strong> matters
  <br />
  from what <strong>sort of</strong> matters
  <br />
  from what <strong>doesn’t</strong> matter at all.’
</h1>

_~ Jason Fried and DHH_

<Notes></Notes>

---

<br />

# 👋

<br />
<br />

## [@robinpokorny](https://robinpokorny.com)

[https://decision-diet-js.now.sh](https://decision-diet-js.now.sh)

<Notes>
  Look for decisions that sort of matter, eliminate them, and you will be more
  creative, productive, and generally better in both life and code.
</Notes>
